...
lenvalid = math.ceil(0.2*len(traind))
...
def weight_update(NN_arch, wt_arch, del_wt_arch, eta, batchsize):
    for layer in range (len(wt_arch)):
        #0 to h 
        wt_arch[layer]=wt_arch[layer] - eta/(batchsize)*del_wt_arch[layer]
    return wt_arch
...
def Backpropagation (data, label, batchsize, NN_arch, wt_arch, eta, maxepoch):
    ...
    counter = 0
    while ((convergence(valacc, wt_arch)==False) and (numepoch<maxepoch)):
...
def Test(data, label, wt_arch):
    ...
    correct = 0
    wrong = 0
    y_pred,_,_ = predict(S, data, wt_arch)
    for i in S:
...
NN_arch = [1] #the NN-architecture list
...
Backpropagation(traind2, labeld2,100, NN_arch, wt_arch, 10, 20) 
...
# x0 = []
# x1 = []
# for i in range(len(traind2)):
#     x0.append(traind2[i][0])
#     x1.append(traind2[i][1])
# print(min(x0)-1, max(x0)+1)
